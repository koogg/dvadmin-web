    ...
    <div ref="tableContainer" class="table">
      ...
      >
        ...
        >
          <template #header>
            <el-button
              v-if="hasPermission('/api/system/dept/:add')"
              type="primary"
              size="small"
              @click="handleCreat()"
            >
          ...
          <template #default="{ row }">
            <div class="ellink">
              <el-link
                v-if="hasPermission('/api/system/dept/:change')"
                :underline="false"
                type="primary"
              ...
              <el-link
                v-if="hasPermission('/api/system/dept/:delete')"
                :underline="false"
                type="danger"
...
async function onSearch() {
  ...
  try {
    // 获取部门数据
    const res = await getDepartList(form);
    // 确保使用正确的字段进行树形结构处理
    dataList.value = handleTree(res.data, "id", "parent", "children");
    loading.value = false;
  } catch (error) {
