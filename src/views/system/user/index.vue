      ...
      <div ref="tableContainer" class="table">
        <el-table :data="dataList" class="el-table" :height="tableMaxHeight" :show-overflow-tooltip="true">
          ...
          <el-table-column label="操作" align="center" fixed="right" min-width="150px">
            <template #header>
              <el-button v-if="hasPermission('/api/user/:add')" type="primary" size="small" @click="handleCreat()">新增</el-button>
            </template>
            <template #default="{ row }">
              <div class="ellink">
                <el-link v-if="hasPermission('/api/user/:change')" :underline="false" type="primary" @click="handleEdit(row)">编辑</el-link>
                <el-link v-if="hasPermission('/api/user/:delete')" :underline="false" type="danger" @click="handleDelete(row)">删除</el-link>
              </div>
            </template>
          </el-table-column>
...
import { getUserList, patchUser, postUser, deleteUser, getListWithDataPermission } from "@/api/user";
import { getDepartList, getRoleList, getAccessibleDepts } from "@/api/system";
...
onMounted(async () => {
  ...
  try {
    const res = await getAccessibleDepts();
    if (res.success) {
      treeData.value = handleTree(res.data, "id", "parent", "children");
    } else {
      // 如果获取可访问部门列表失败，回退到普通部门列表
      const deptRes = await getDepartList();
      treeData.value = handleTree(deptRes.data, "id", "parent", "children");
    }
  } catch (error) {
